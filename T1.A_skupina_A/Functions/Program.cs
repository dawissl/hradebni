...
namespace Functions
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            int input_8_A = 18;
            int input_8_B = 5;
            bool result_8_A = Function8(input_8_A, input_8_B);
            bool result_8_B = Function8(input_8_B, input_8_A);
        ...
        private static int Function11(string veta, char a)
        {
            int g = 0;
            if (char.IsDigit(a)) throw new Exception("cislo");
            string letter = a.ToString().ToUpper();
            foreach (char s in veta.ToUpper())
            {
                if (char.Parse(letter) == s) g++;
            }
            return g;
        }
